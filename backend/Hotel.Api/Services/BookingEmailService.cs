...
public class BookingEmailService : IBookingEmailService
{
    ...
    {
        var customer = booking.Customer!;
        var roomNumber = booking.Room?.RoomNumber ?? "Will be assigned after payment";
        var roomTypeName = booking.Room?.RoomType?.Name ?? booking.RoomType?.Name ?? "-";

        return $"""
        <html>
        <body style="font-family:Arial,sans-serif;color:#111827">
          <h2>Booking confirmed</h2>
          <p>Halo {WebUtility.HtmlEncode(customer.Name)}, pembayaran berhasil dan booking Anda sudah dikonfirmasi.</p>
          <table cellpadding="6" cellspacing="0" style="border-collapse:collapse">
            ...
            <tr><td><strong>Kamar</strong></td><td>{WebUtility.HtmlEncode(roomNumber)} - {WebUtility.HtmlEncode(roomTypeName)}</td></tr>
